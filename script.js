...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function getOperatorImagePath(operatorName) {
        ...
        return `images/operators/${normalized}.jpg`;
    }
    ...
    function renderMap(mapId) {
        ...
        // Renderiza Carrossel de Operadores Recomendados para o mapa atual
        const recommendedOperators = mapData.recommendedOperators || mapData.operators.attack.slice(0, 3);
        recommendedCarouselEl.innerHTML = "";

        if (carouselDescriptionEl) {
            carouselDescriptionEl.textContent = `${mapData.name}: operadores recomendados por mapa.`;
        }

        recommendedOperators.forEach(op => {
            const card = document.createElement("article");
            card.className = "carousel-card";

            const img = document.createElement("img");
            img.className = "carousel-img";
            img.src = getOperatorImagePath(op.name);
            img.alt = op.name;
            img.onerror = () => {
            ...
            body.innerHTML = `
                <span class="operator-role">Ataque</span>
                <h3>${op.name}</h3>
                <p>${op.desc}</p>
            `;
            ...
            recommendedCarouselEl.appendChild(card);
        });
    }
    ...
    function getCarouselCards() {
        ...
        if (!carousel) return [];
        return Array.from(carousel.querySelectorAll(".carousel-card"));
    }
